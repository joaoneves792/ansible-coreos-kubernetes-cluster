...
def etcdIP(num)
  return "10.0.0.#{num+50}"
end

def controllerIP(num)
  return "10.0.0.#{num+100}"
end

def workerIP(num)
  return "10.0.0.#{num+200}"
end
...
Vagrant.configure("2") do |config|
  ...
  (1..$etcd_count).each do |i|
    config.vm.define vm_name = "e%d" % i do |etcd|
      ...
      etcd.vm.network :public_network, ip: etcdIP(i), bridge: "enp4s0"
  ...
  (1..$controller_count).each do |i|
    config.vm.define vm_name = "c%d" % i do |controller|
      ...
      controller.vm.network :public_network, ip: controllerIP, bridge: "enp4s0"


      #controller.vm.provision :file, :source => "install-python-on-coreos.sh", :destination => "/home/core/python"
  ...
  config.vm.provision :ansible do |ansible|
          ansible.host_vars = {}
          (1..$etcd_count).each do |i|
            ansible.host_vars["e%d" % i] = {"ansible_python_interpreter" => "'/home/core/bin/python'" , "public_ip" => etcdIP(i) } 
          end
          (1..$controller_count).each do |i|
            ansible.host_vars["c%d" % i] = {"ansible_python_interpreter" => "'/home/core/bin/python'", "public_ip" => controllerIP(i) } 
          end
          (1..$worker_count).each do |i|
            ansible.host_vars["w%d" % i] = {"ansible_python_interpreter" => "'/home/core/bin/python'", "public_ip" => workerIP(i) }
          end
        
        ansible.groups = { 
          "etcds" => [*1..$etcd_count].map{ |ei| "e%d" % ei },
          "controllers" => [*1..$controller_count].map{ |ci| "c%d" % ci },
          "workers" => [*1..$worker_count].map{ |wi| "w%d" % wi }
        }
        ansible.playbook = "./kubernetes-playbook.yaml"
      end
